...
HotelManagement::HotelManagement() {
    roomTypeMap[static_cast<RoomType>(0)] = "Luxury";
    roomTypeMap[static_cast<RoomType>(1)] = "Single";
    roomTypeMap[static_cast<RoomType>(2)] = "Multi";
}
...
void HotelManagement::registerGuest(const std::string &guestName, int roomNumber, const std::string &registrationDate,
                                    int days) {
    for (auto it = rooms.begin(); it != rooms.end(); ++it) {
        if (it->getRoomNumber() == roomNumber) {
            if (it->getType() == static_cast<RoomType>(2)) {
                std::cout << "Room " << roomNumber << " is Multi room. Use 5th option to register in multi room" << std::endl;
                return;
            }
            if (!it->isOccupied()) {
        ...
        if (it->getRoomNumber() == roomNumber) {
            if (dynamic_cast<MultiRoom *>(it.operator->()) != nullptr) {
                ...
                if (multiRoom->getGuestCount() < multiRoom->getMaxGuests()) {
                    ...
                    std::cout << "Guest " << guestName << " successfully registered in room " << roomNumber
                              << std::endl;
                    return;
                } else {
...
void HotelManagement::listAvailableRooms() const {
    ...
    for (auto it = rooms.begin(); it != rooms.end(); ++it) {
        if (!it->isOccupied() && it->getType() != static_cast<RoomType>(2)) {
            std::cout << "Room " << it->getRoomNumber() << " (" << roomTypeMap.find(it->getType())->second << ")" << std::endl;
        }
    }
}
...
void HotelManagement::listMultiRooms() const {
    ...
    for (auto it = rooms.begin(); it != rooms.end(); ++it) {
        if (dynamic_cast<const MultiRoom *>(it.operator->()) != nullptr) {
            ...
            std::cout << "Room " << multiRoom->getRoomNumber() << " (" << roomTypeMap.find(multiRoom->getType())->second << "): ";
            if (multiRoom->isFull()) {
                std::cout << "Full";
